...
/*   Updated: 2023/12/11 19:21:35 by fde-alen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_text(int fd, char *line_buffer)
{
	...
	while (!ft_strchr(line_buffer, '\n') && read_size != 0)
	{
		...
		line_buffer = ft_strjoinn(line_buffer, read_buffer);
	}
	free(read_buffer);
	return (line_buffer);
}
...
static char	*extract_remaining(char *line_buffer)
{
	...
	remaining_buffer = malloc(sizeof(char) * (ft_strlenn(line_buffer) - i + 1));
	if (!remaining_buffer)
		return (NULL);
	i++;
	while (line_buffer[i])
